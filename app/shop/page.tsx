...
export default function PhonesPage() {
  ...
  useEffect(() => {
    ...
    const uniqueRam = Array.from(
      new Set(phones.flatMap((phone) => phone.specs.ram))
    );
    setRam(uniqueRam);

    const uniqueStorage = Array.from(
      new Set(phones.flatMap((phone) => phone.specs.storage))
    );
  ...
  const applyFilters = (
    ...
    const filtered = phones.filter((phone) => {
      ...
      const ramMatch =
        ram.length === 0 || ram.some((r) => phone.specs.ram.includes(r));
      const storageMatch =
        storage.length === 0 ||
        storage.some((s) => phone.specs.storage.includes(s));
                      ...
                      {storage.map((storageSize) => (
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={() =>
                              handleStorageChange(storageSize)
                            }
                          />
                          <label
